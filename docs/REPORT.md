...
| Phase 39 — Phase 3 + 1,400 synthetic rare constructions (22% mix) | 79.9 | 53.7 | 28.4 | 41.8 | 12.7 |
...
**The empirical generalisation: at 296M / 6 epochs, rearranging the same supervision plateaus or regresses; orthogonal information sources unlock gain — but only when the new information's distribution matches the test distribution.** The architectural pattern is robust across encoder-side conditioning (Phase 2), input-side augmentation (Phase 3), output-side hierarchical decoders (Phases 5, 6), and input-side dynamic relational reasoning (Phase 3.1). On the data side, Phase 39 (synthetic rare-construction augmentation at 22% of corpus) tested whether template-based new training data unlocks similar gains: it produces a uniform +0.4 pp improvement across all four metrics on MASAQ Quranic (where the synthetic distribution matches the test surface — *kāna sisters*, *istithnāʾ*, *mawṣūl* are higher-density), but a sharp regression on Gazelle MSA news (case −3.0, role-F1 −12.9, marker −3.0, fully −7.4). The role head overfit to template surfaces because synthetic at 22% mix dominated the in-distribution training signal. Phase 39 v2 is documented as the next iteration — with reduced ratio (5-8% not 22%), 3-4× more lexical filler diversity, sentence-shape variation per template, and per-construction eval. The thesis generalises: *new information must match the test distribution to unlock gain*; template-narrow synthetic data shifts the role head's prediction surface and hurts in-distribution metrics. Phase 3.1 is informative because it tests whether *richer relational reasoning over the same dep tree* would unlock further gain on top of Phase 3's static features — and it doesn't. The encoder representation Phase 3-A learns from the static dep features already saturates the dep information at this corpus size; adding attention on top just redistributes it. Production lineage is rev 2 → Phase 1 → **Phase 3-A**; Phases 4a, 2, 5, 6, 3.1 ship as opt-in archival. Future architectural levers must add genuinely new information (rare-construction augmentation, cleaner Stanza coverage, gold UD-PADT dep on the morph half, inference-time Stanza), not new downstream mechanisms.
...
#### 5.6.6 Phase R, Phase R2, and the forward-path correction (negative result; evaluator fix is the real contribution)

After Phase 3-A shipped, we tested whether *inference-side* mechanisms could lift Phase 3-A further on the catastrophic Gazelle constructions (kāna sisters, istithnāʾ, quranic-proxy — each at 0% *fully* in the original per-construction breakdown). The per-construction baseline ran on top of Phase 3-A and revealed the bottleneck was localised to a few sparse high-reasoning families: kāna sisters 0/7 *fully* with calibration gap −0.533, istithnāʾ 0/5 *fully*, while iḍāfa was already well-handled (33% *fully*, better than overall 25%).

**Phase R — retrieval over a per-construction grammar memory.** We built a 18,839-instance grammar memory across 7 construction families (`kana_sisters`, `inna_sisters`, `istithna`, `mawsool`, `idafa`, `idafa_multi`, `quranic_proxy`), keyed on construction-span signatures (head morph, head DEPREL, head governor POS) and indexed by FAISS cosine over Phase 3-A's pooled-iʿrāb encoder embeddings. At inference, the predictor detects construction spans (surface particle + role-based for iḍāfa), retrieves top-k analogues, aggregates a soft per-position prior over (case, role, marker), and adds λ·log(prior) to the iʿrāb logits. The retrieval pool is symbolically filtered by family + particle group before vector ranking. R-C (λ=0.3, k=5) lifted MASAQ uniformly (+0.7 *fully* overall, +8.5 pp on `idafa_multi` specifically) but failed every Gazelle gate (kāna 0%→0%, calibration worsened −0.533 → −0.884). Same Gazelle-vs-MASAQ asymmetry as Phase 39: when the retrieval pool's distribution matches the test surface (MASAQ Quranic / mixed registers), retrieval transfers useful structural information; when it doesn't (Gazelle MSA-news has different per-construction densities), retrieval pushes the model toward a wrong consensus.

**Phase R2 — retrieval-guided structural reasoning with confidence gating.** Per-construction reasoners (KanaReasoner, IstithnaReasoner, MawsoolReasoner, InnaReasoner, QuranicProxyReasoner) consume retrieved analogues and apply a three-tier gating: τ_high (override the consensus label as a one-hot logit), τ_med (additive log-odds bias), τ_low (fallback to Phase 3-A). Four iterations were tested:

- **R2 v1** (consensus-only): Gazelle kāna stayed 0%; calibration worsened.
- **R2 v2** (canonical case rule + base-confidence guard + harf-particle protection + log-odds formula correction): kāna case regressed −14.3 pp because the canonical rule "khabar of kāna is *naṣb*" forced *naṣb* on positions where the khabar was a nested nominal sentence (case = *rafʿ* via embedded mubtada).
- **R2 v2.1** (canonical case applied only when consensus agrees): Gazelle overall *fully* +1.0, role +2.8; MASAQ kāna *fully* +0.7 with calibration sign-flipped from −0.063 to +0.013. The improvements appeared real and prompted documentation as the new best hybrid system.
- **Forward-path collateral diagnostic.** Investigating two small Gazelle regressions (inna *marker* −9.1, iḍāfa *fully* −2.1) revealed that *all 10 word-level prediction flips between Phase 3-A and R2-v2.1 occurred outside detected construction spans*. The R2-v2.1 predictor was re-implementing the encoder forward path manually — calling `model.encoder(...)` then `_word_first_pool(...)` directly — instead of calling `model.forward(...)`. The manual path produced subtly different logits from the model's actual forward (the model uses `_encode_and_pool(...)` which applies `self.dropout(pooled)` after pooling, with extra ordering effects). Both the apparent gains and the apparent collateral were artifacts of this drift.
- **R2 v2.1.1** (replace manual forward with `model.forward(...)`): the previously-claimed gains and previously-observed collateral *both* vanish to 0.0 on every metric on every construction on both surfaces. The KanaReasoner mechanism still fires (2 spans on Gazelle, 33 spans on MASAQ), but its overrides land on words where Phase 3-A is already producing the same answer. Net signal contributed by the inference-side reasoner: zero.

**Evaluator fix — the real contribution from this debugging cycle.** The Phase 3-A per-construction baseline reported `kana_sisters fully = 0%` on Gazelle. Trace inspection revealed the cause was not the model but the *gold extractor*: `evaluation/structural.py` matched only literal "اسم كان" and "خبر كان" prose, while Gazelle uses particle-specific phrasing ("اسم ليس", "اسم صار", "خبر (ليس)", "والجملة الاسمية خبر صار"). 4 of 6 Gazelle kāna span words had `gold role = None`, blocking *fully* at 0% regardless of model performance. We added `_detect_kana_role()` which runs before the generic role-pattern scan and recognizes "اسم/خبر <kana_particle>" for any of 12 kāna-family particles (with optional parenthesized form), taking priority over surface-case roles like *ism majrūr* (so "اسم مجرور لفظا، منصوب محلا على أنه خبر (ليس)" now correctly produces `khabar_kana`, the operative grammatical role, not the surface case description). With the same Phase 3-A checkpoint and the corrected evaluator, Gazelle `kana_sisters` *fully* moves from artificial 0% to **14.3%** and role from 0% to **57.1%**; calibration gap improves from −0.533 to −0.189. The model has been doing kāna reasoning correctly all along — the evaluator was hiding it.

**Final corrected Gazelle metrics (Phase 3-A on FIXED evaluator, n=107).**
...
**Final corrected MASAQ metrics (Phase 3-A on FIXED evaluator, n=5,007).**
...
**Final synthesis: what works versus what plateaus at this scale.**

| intervention | category | result vs Phase 3-A |
|---|---|---|
| Phase 1 morphology supervision | orthogonal info | **+5.4 role-F1, +1.5 *fully*** ✓ |
| Phase 3 static UD dep features | orthogonal info | **+3.0 case, +3.8 marker, +0.7 *fully*** ✓ (PRODUCTION) |
| Evaluator fix (kāna role extraction) | measurement | **+14.3 Gazelle kāna *fully*** ✓ (real measurement gain, model unchanged) |
| Phase 4a taxonomy expansion (25→34) | rearrangement | flat / partial substitution with morph |
| Phase 2 soft conditioning (FiLM/additive/detached) | rearrangement | flat / regress |
...
| Phase 39 synthetic augmentation (22% mix) | data, distribution mismatch | Gazelle −12.9 role, MASAQ +0.4 |
| Phase R-C retrieval + soft logit bias | inference-side | Gazelle regress, MASAQ partial |
| Phase R2 reasoning + override | inference-side | **0.0 everywhere** (post forward-path fix) |

Two interventions produced robust gains: morphology supervision and dependency features. Both add orthogonal linguistic information to the encoder input. Eight interventions (taxonomy / conditioning / hierarchy / relational attention / synthetic-mismatch / retrieval-only / inference-side reasoning) plateau or regress. The bottleneck at this scale is information coverage and supervision quality, not architecture. The R2 cycle is reported as a clean negative result with three deliverables: (a) the per-construction evaluation framework (`scripts/structured/eval_per_construction.py`) which any future intervention can reuse; (b) the grammar-memory infrastructure (signature, retrieval, FAISS index, structural reasoners) preserved as archival code for the case study; (c) the kāna-aware evaluator fix in `evaluation/structural.py`, which is real and permanent.

**Honest reading of the R2 cycle.** The R2 framework is internally consistent — three confidence tiers, log-odds biasing, base-confidence guard, harf-particle protection, canonical-case rule with consensus agreement — but at this scale it provides zero measurable benefit on top of Phase 3-A. The KanaReasoner override fires on detected kāna spans; the consensus label it would emit either matches Phase 3-A's argmax already (no change), or it disagrees but the disagreement reflects the same retrieval-pool distribution mismatch that bound Phase R-C and Phase 39. The override neither helps nor hurts when the forward path is correct. We document this as a strict negative result: at 296M parameters / 6 epochs / a ~7K-sentence supervised corpus, *both* training-time rearrangement and inference-time symbolic reasoning have been thoroughly explored, and the productive lever lies elsewhere — in the orthogonal-information directions outlined in §8.
...
The Phase 1 → Phase 4a → Phase 2 → Phase 3 → Phase 3.1 → Phase 5 → Phase 6 → Phase 39 → Phase R-C → Phase R2 sequence has thoroughly explored architectural and inference-side levers at the 296M / 6-epoch / ~7K-sentence-corpus envelope. The empirical pattern is overwhelming: rearrangement plateaus, inference-side reasoning is neutral, distribution-mismatched data hurts. At this scale, further gains require **information coverage and supervision quality**, not architectural rearrangement. The directions below are ordered by expected leverage.

1. **Larger and richer corpora.** The training corpus is 77K Haiku-distilled word-rows from ~5K MSA sentences. The Haiku-distillation ceiling (case 67.2%) caps every open-weight student trained on it (§5.2(a)). Two complementary expansions: (a) a Sonnet-distilled corpus at the same scale (~$15–20 for 5K rows) would test whether the parametric capacity null result is teacher-bound; (b) integrating CamelTB (188K multi-register words), I3rab (601 MSA sentences), and the Extended Quranic Treebank (132K Quranic tokens) after schema conversion from CATiB to our (case, role, marker) tuples would yield ~150–250K MSA + ~80–130K Quranic words. The cross-register asymmetry observed throughout this work (Phase 39, Phase R-C, Phase R2 all show MASAQ-vs-Gazelle distribution mismatches) is the strongest single signal that distribution coverage is the binding constraint.

2. **Richer annotation.** The 25-label canonical role taxonomy is a deliberate collapse of 590 distinct gold strings; even the Phase 4a expansion to 34 labels still maps long-tail role strings (*مفعول مطلق*, *منصوب لفظا مرفوع محلا*) to coarse categories. Hand-curated annotation that preserves the long-tail role string surface would close the residual ~6–10 pp gap to seq2seq prose decoding (§5.6, §7.8). Per-word morph + dep + role + marker annotation on 1–2K Gazelle-style MSA sentences would also enable construction-stratified analysis above the n=134 detectability floor.

3. **Broader rare-construction coverage.** Even after the evaluator fix, *istithnāʾ* and `quranic_proxy` remain at 0% *fully* on Gazelle (5 + 8 words). Phase 39 attempted template-based synthetic augmentation but the narrow templates at 22% mix overfit the role head and hurt Gazelle in-distribution metrics. A targeted ~500 hand-annotated examples per failing construction (kāna sisters, istithnāʾ, mawṣūl, quranic-proxy verbal patterns) added to the training corpus — without templating — would directly probe whether these failures are coverage-bound or structurally harder. Combined with the existing per-construction evaluation framework (`scripts/structured/eval_per_construction.py`), this is testable in a single retraining cycle.

4. **Better dependency quality.** Phase 3-A uses Stanza's Arabic UD parser (UAS ≈ 84%) and the alignment script keeps records where ≥50% of whitespace tokens align to UD tokens; only 70% of distill_v2 records ended up with `has_dep=True`. Three concrete improvements: (a) drop the alignment threshold to 30% to lift coverage, (b) add gold UD-PADT dep on the morph-only half of the corpus (currently `has_dep=False`), (c) run Stanza at inference time on Gazelle/MASAQ inputs (the current predictor passes zero `dep_emb` at inference, since Stanza isn't run in the predictor's hot path). The fact that even noisy 70%-coverage Stanza dep features produced +3.0 pp case improvement suggests cleaner dep coverage would compound the gain.

5. **Stronger pretrained Arabic encoders.** Phase 3-A uses AraT5v2-base (296M, MSA-pretrained). Two architectures emerged after this work began that warrant testing: AraBART (139M, encoder-decoder, multi-dialect pretraining) and CAMeLBERT-CA (~135M, classical-Arabic-specialized). For the per-word classification setup (encoder + 4 heads), classical-Arabic specialization should lift MASAQ Quranic role-F1 (currently 17.1%) materially. Since the Phase 1 + Phase 3 supervision recipe transfers cleanly across encoders (the iʿrāb heads are encoder-agnostic), this is a pure-substitution experiment.

6. **Longer training schedules.** Every checkpoint in this study trained for 6 epochs at 296M parameters on ~77K rows. Rev 2 reached CE ≈ 2 within 6 epochs; the CRF stress test (§5.6.5 epilogue) plateaued at NLL ≈ 14, suggesting structured losses need more epochs to saturate. A 12–18 epoch retrain of Phase 3-A at the existing recipe would test whether supervision saturation rather than architecture is the role-F1 ceiling.

7. **Larger-scale supervision via multi-task learning.** Diacritization (Sadeed; Aldallal et al., 2025) and iʿrāb are structurally linked: the case marker predicted in iʿrāb directly determines a word's final vowel. Our self-consistency analysis suggests grammatical reasoning constrains diacritization in approximately 96% of cases. A multi-task formulation that jointly optimizes diacritization and iʿrāb objectives — exposing the model to ~10–100× more diacritized text — could lift the encoder representation in a way that orthogonally complements the morph + dep input augmentations validated here.

We deliberately do **not** include further architectural rearrangements (additional heads, conditioning variants, hierarchy experiments, structural override mechanisms) in the future-work agenda. The empirical evidence in §5.6.6 is overwhelming that this lever is exhausted at the current scale.
